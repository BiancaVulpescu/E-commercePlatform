...
namespace SmartE_commercePlatform.Controllers
{
    ...
    public class ShoppingCartsController(IMediator mediator) : ControllerBase
    {
        ...
        public async Task<ActionResult<IEnumerable<ProductDtoMinimal>>> GetAllProductsByShoppingCartId([FromRoute] Guid id)
        {
        ...
        public async Task<ActionResult> AddProductToShoppingCart([FromRoute] Guid shoppingCartId, [FromRoute] Guid productId)
        {
            return (await mediator.Send(new AddProductToShoppingCartCommand { ProductId = productId, ShoppingCartId = shoppingCartId }))
                .Match<ActionResult>(
                    _ => NoContent(),
                    error => BadRequest(error)
                );
