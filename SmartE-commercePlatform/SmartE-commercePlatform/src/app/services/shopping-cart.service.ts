...
export class ShoppingCartService {
  ...
  createOrder(order: { city: string; address: string; status: string }): Observable<string> {
    ...
    return this.http.post<string>(`${this.orderApiUrl}/orders`, orderWithToken, { headers });
  }
  ...
  addProductsToOrder(orderId: string, products: { productId: string; quantity: number }[]): Observable<any> {
    ...
    return from(products).pipe(
      concatMap(product =>
        this.http.put(`${this.orderApiUrl}/orders/${orderId}/products/${product.productId}?quantity=${product.quantity}`, {}, { headers })
      )
    );
  }
}